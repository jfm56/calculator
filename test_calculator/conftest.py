...
def generate_test_data(num_records):
    """Operation mapping for both Calculator and Calculation tests"""
    operation_mapping = {
        'add': add,
        'subtract' : subtract,
        'multiply' : multiply,
    ...
    for _ in range(num_records):
        ...
        if operation_func == divide:
            b = Decimal('1') if b ==('0') else b

        try:
            if operation_func == divide and b == Decimal('0'):
                excepted = "ZeroDivisionError"
            else:
                expected = operation_func(a, b)
        except ZeroDivisionError:
            expected = "ZeroDivisionError"

        yield a,b, operation_name, operation_func, excepted

def pytest_addoption(parser):
    """Allows pytest to res=conize --num_record argument"""
    parser.addoption("--num_record", action="store", default=5, type=int, help="Number of test records to generate")

def pytest_generate_tests(metafunc):
    # Chec if the test is expecting any of the dynamically generates fixtures
